  ...
  describe('after adding a promise', function() {
    ...
    describe('events', function() {
      ...
      it('should fire "start" with param when promise is added', function() {
        ...
        expect(spies.start).toHaveBeenCalledWith(3);
      });
      it('should fire "done" with param when promise is resolved or rejected', function() {
        ...
        expect(spies.error).not.toHaveBeenCalled();
        expect(spies.success).not.toHaveBeenCalled();
        expect(spies.start).toHaveBeenCalledWith("fun!");
        deferred.resolve(11);
        digest();
        expect(spies.done).toHaveBeenCalledWith(11);
        expect(spies.success).toHaveBeenCalledWith(11);
        deferred = $q.defer();
        myTracker.addPromise(deferred.promise);
        deferred.reject(22);
        digest();
        expect(spies.done).toHaveBeenCalledWith(22);
        expect(spies.error).toHaveBeenCalledWith(22);
      });
      it('should fire "error" with param when promise is rejected', function() {
        ...
        deferred.reject(7);
        digest();
        expect(spies.error).toHaveBeenCalledWith(7);
      });
      it('should fire "success" with param when promise is resolve', function() {
        ...
        deferred.resolve(9);
        digest();
        expect(spies.success).toHaveBeenCalledWith(9);
      });
      it('should fire all events, at right times', function() {
        var deferred = $q.defer();
        myTracker.addPromise(deferred.promise, 5);
        expect(spies.start).toHaveBeenCalledWith(5);
        deferred.resolve(6);
        digest();
        expect(spies.start.callCount).toBe(1);
        expect(spies.success).toHaveBeenCalledWith(6);
        expect(spies.error).not.toHaveBeenCalled();
        expect(spies.start).toHaveBeenCalledWith(5);
        deferred = $q.defer();
        myTracker.addPromise(deferred.promise, 7);
        expect(spies.start).toHaveBeenCalledWith(7);
        expect(spies.start.callCount).toBe(2);
        deferred.reject(8);
        digest();
        expect(spies.done).toHaveBeenCalledWith(8);
        expect(spies.error).toHaveBeenCalledWith(8);
        expect(spies.start.callCount).toBe(2);
